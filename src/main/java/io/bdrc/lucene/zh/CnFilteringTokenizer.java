...
/** 
 * Copy of StandardTokenizer that only returns Chinese tokens
 * 
 */

public final class CnFilteringTokenizer extends Tokenizer {
   ...
   */
  public CnFilteringTokenizer(AttributeFactory factory) {
    super(factory);
    init();
  }
  ...
  public final boolean incrementToken() throws IOException {
    ...
    while(true) {
      ...
      if (scanner.yylength() <= maxTokenLength) {
        ...
        /* ignore all tokens that are not Chinese */
        if (tokenType != CJ && tokenType != IDEOGRAPHIC) {
            termAtt.setEmpty();
            continue;
        }
        offsetAtt.setOffset(correctOffset(start), correctOffset(start+termAtt.length()));
        typeAtt.setType(CnFilteringTokenizer.TOKEN_TYPES[tokenType]);
        return true;
      } else
